...
def quiz(request, subject):
	...
	quiz_questions = Question.objects.filter(course__subject_name=subject).all().order_by('?')[:30]
	...
	return render(request, 'quiz/quiz.html',{'subject': subject, 'section': 'quiz',
								'total': total, 'quiz_questions': quiz_questions})
